...
export function VaultProvider({ children }) {
  ...
  // Download file
  const downloadFile = async (fileId, fileName, encryptionKey, encrypted = false) => {
    if (!currentUser) throw new Error('User not authenticated');
    ...
    try {
      const { file: blob, metadata } = await supabaseService.downloadFile(
        fileId, 
        currentUser.id,
        encrypted ? null : encryptionKey // Don't decrypt if encrypted = true
      );
      
      const downloadFileName = encrypted ? 
        `${fileName || metadata.file_name}.encrypted` : 
        fileName || metadata.file_name;
      
      supabaseService.createDownloadLink(blob, downloadFileName);
      return true;
    } catch (error) {
