...
function Vault({ encryptionKey }) {
  ...
  const handleDownload = async (file, encrypted = false) => {
    try {
      await downloadFile(file.id, file.file_name, encryptionKey, encrypted);
    } catch (error) {
        ...
        {files.map(file => (
          <div key={file.id} className="file-card">
            ...
            <div className="file-actions">
              ...
              <button 
                className="download-btn encrypted"
                onClick={() => handleDownload(file, true)}
                title="Download encrypted version"
              >
                🔒 Download Encrypted
              </button>
              <button 
